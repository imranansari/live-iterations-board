var UpdatingEpicStoryBoardCollectionView = UpdatingCollectionView.extend({
    ...
    render : function() {
        ...
        _(this._childViews).each(function(childView) {
            ...
            var phase = childView.model.toJSON().phase;
            //$('#story_' + phase + ' ul').append(childView.render().el);
            var myphase = $('#myEpic1').parent().parent().parent().find('.stories '+ '#story_' + phase + ' ul');
            myphase.append(childView.render().el);
            console.log(myphase);
        });
